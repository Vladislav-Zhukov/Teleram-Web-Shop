...
async def add_name(m: Message, state: FSMContext):
    ...
    await m.answer("Введите количество (stock >= 0):")
...
async def add_stock(m: Message, state: FSMContext):
    try:
        ...
        await m.answer("Введите корректное число >= 0")
        return
...
async def add_price(m: Message, state: FSMContext):
    try:
        ...
        await m.answer("Введите корректную цену >= 0")
        return
...
async def add_photo_and_create(m: Message, state: FSMContext):
    ...
    try:
        ...
        await m.answer(f"Товар создан: #{created['id']} {created['name']}")
